...
@dsl.component(
    ...
    packages_to_install=[
        ...
        "joblib", "optuna", "kfp-pipeline-spec>=0.1.16"
    ]
)
...
def training_pipeline(
    ...
    prepare_opt_data_task = prepare_opt_data_op(
        gcs_bucket_name=GCS_BUCKET_NAME, pair=pair, timeframe=timeframe,
    ...
    optimize_lstm_task = optimize_lstm_op(
        ...
        pair=pair, timeframe=timeframe, n_trials=n_trials,
        output_gcs_prefix=f"gs://{GCS_BUCKET_NAME}/params/LSTM_v2",
        project_id=PROJECT_ID
    )
    optimize_lstm_task.set_display_name("Optimize_LSTM_Hyperparameters_CPU_Only")
    optimize_lstm_task.set_cpu_limit("8").set_memory_limit("30G")

    # Configuración de GPU comentada
    # if kfp_opt_accelerator_type != "ACCELERATOR_TYPE_UNSPECIFIED" and kfp_opt_accelerator_count > 0:
    #     with dsl.Condition(kfp_opt_accelerator_type != "ACCELERATOR_TYPE_UNSPECIFIED" and kfp_opt_accelerator_count > 0, name="Set_Optuna_GPU"):
    #         optimize_lstm_task.set_gpu_limit(kfp_opt_accelerator_count)
    ...
    train_lstm_task = train_lstm_vertex_ai_op(
        project_id=PROJECT_ID, region=REGION, gcs_bucket_name=GCS_BUCKET_NAME,
        params_path=optimize_lstm_task.outputs['Output'], # CAMBIO: Usar .outputs['Output']
        pair=pair, timeframe=timeframe,
    ...
    decide_task = decide_promotion_op(
        new_backtest_metrics_dir=backtest_task.outputs['Output'], # CAMBIO: Usar .outputs['Output']
        new_lstm_artifacts_dir=train_lstm_task.output,
        new_rl_model_path=train_rl_task.output,
        gcs_bucket_name=GCS_BUCKET_NAME, pair=pair, timeframe=timeframe, project_id=PROJECT_ID,
        current_production_metrics_path=f"gs://{GCS_BUCKET_NAME}/models/production/{pair}/{timeframe}/metrics_production.json",
    ...
    # CAMBIO: Usar dsl.If en lugar de dsl.Condition
    with dsl.If(decide_task.output == True, name="If_Model_Promoted"):
        notify_promoted_task = notify_pipeline_status_op(
            ...
            pipeline_run_status="SUCCESS", model_promoted=True
        )
    ...
    with dsl.If(decide_task.output == False, name="If_Model_Not_Promoted"):
        notify_not_promoted_task = notify_pipeline_status_op(
            project_id=PROJECT_ID, pair=pair, timeframe=timeframe,
            pipeline_run_status="SUCCESS", model_promoted=False
        )
...
if __name__ == "__main__":
    ...
    if SUBMIT_TO_VERTEX_AI:
        ...
        run_pair = DEFAULT_PAIR
        run_timeframe = DEFAULT_TIMEFRAME
        run_n_trials = 1
        run_backtest_features_path = f"gs://{GCS_BUCKET_NAME}/backtest_data/{run_pair}_{run_timeframe}_unseen.parquet"

        default_polygon_secret_name = "polygon-api-key"
        ...
        pipeline_parameter_values = {
            ...
            "polygon_api_key_secret_name_param": default_polygon_secret_name,
            "polygon_api_key_secret_version_param": "latest",
        }

        job_display_name = f"algo-trading-v2-secret-cpu-{datetime.now().strftime('%Y%m%d%H%M%S')}"
        ...
        try:
            ...
            if job_resource_name:
                 ...
                 print(f"  Estado actual del Job (si disponible): {current_job_state}")
                 print(f"  Error del Job (si disponible): {current_job_error}")
            ...
            if job_resource_name:
                ...
                print(f"\n🔗 Visualizar en Vertex AI Pipelines: https://console.cloud.google.com/vertex-ai/pipelines/runs/{job_id}?project={PROJECT_ID}")
            else:
