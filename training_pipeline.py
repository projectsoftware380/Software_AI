...
# --- Docker Image URIs ---
# Using a fixed tag for easier debugging and synchronization with image build process.
# Ensure images with this tag are built and pushed to Artifact Registry.
KFP_COMPONENT_TAG = "latest"  # Fixed tag
KFP_COMPONENTS_IMAGE_URI = (
    f"europe-west1-docker.pkg.dev/{PROJECT_ID}/data-ingestion-repo/data-ingestion-agent:{KFP_COMPONENT_TAG}" #
)

# Using a fixed tag for easier debugging.
VERTEX_LSTM_TRAINER_TAG = "latest"  # Fixed tag
VERTEX_LSTM_TRAINER_IMAGE_URI = (
    f"us-central1-docker.pkg.dev/{PROJECT_ID}/trading-images/runner-lstm:{VERTEX_LSTM_TRAINER_TAG}" #
)
...
def update_data_op(
    ...
    logger.info(f"Executing on KFP pod: {os.environ.get('HOSTNAME', 'N/A')}")
    try:
        with open("/proc/self/cgroup", "r") as f:
            cgroup_info = f.readline().strip()
            logger.info(f"Container cgroup info (first line): {cgroup_info}")
    except Exception as e_cgroup:
        logger.warning(f"Could not read /proc/self/cgroup: {e_cgroup}")

    try:
        metadata_url = "http://metadata.google.internal/computeMetadata/v1/instance/service-accounts/default/email" # Corrected URL
    ...
    if process.returncode != 0:
        ...
        try:
            ...
            if best_score_line:
                ...
                if len(parts) > 1:
                    score_value_str = ''.join(filter(lambda x: x.isdigit() or x == '.' or x == '-', parts[1].strip().split()[0]))
...
def train_lstm_vertex_ai_op(
    ...
    for handler_idx in range(len(logging.root.handlers)):
    ...
    logger.info(f"  Vertex training image URI: {vertex_training_image_uri}")
    ...
    custom_job_launch_time_utc = datetime.now(timezone.utc)
    try:
    ...
    while time.time() - start_poll_time < max_poll_wait_sec:
        ...
        for page in blobs.pages:
            if hasattr(page, 'prefixes') and page.prefixes:
                for dir_prefix_full_path in page.prefixes:
                    model_blob_path = f"{dir_prefix_full_path.rstrip('/')}/model.h5"
    ...
    err_msg = f"LSTM model directory (containing model.h5) not found in gs://{expected_bucket_name}/{gcs_listing_prefix} after {max_poll_wait_sec / 60:.1f} min. Vertex Job: {custom_job.display_name if custom_job else 'N/A'}" #
...
def decide_promotion_op(
    ...
    if process.returncode == 0:
        ...
        if "PROMOVIDO a producción." in process.stdout or "Model promoted to production".lower() in process.stdout.lower(): #
...
def notify_pipeline_status_op(
    ...
    try:
        ...
        if status_summary not in ["SUCCEEDED", "FAILED", "CANCELLED"]: 
            logger.warning(
                f"Pipeline status '{pipeline_run_status}' (normalized to '{status_summary}') is not one of the primary expected end states " #
                "(SUCCEEDED, FAILED, CANCELLED). Treating unexpected as FAILED for notification."
            )
            if "SUCCEEDED" not in status_summary and "SUCCESS" not in status_summary : 
        ...
        details = {
            ...
            "model_promoted_status": model_promoted if status_summary == "SUCCEEDED" else False,
...
# --- Set ImagePullPolicy for Components ---
update_data_op.component_spec.implementation.container.image_pull_policy = "Always" #
prepare_opt_data_op.component_spec.implementation.container.image_pull_policy = "Always" #
optimize_lstm_op.component_spec.implementation.container.image_pull_policy = "Always" #
prepare_rl_data_op.component_spec.implementation.container.image_pull_policy = "Always" #
train_rl_op.component_spec.implementation.container.image_pull_policy = "Always" #
backtest_op.component_spec.implementation.container.image_pull_policy = "Always" #
decide_promotion_op.component_spec.implementation.container.image_pull_policy = "Always" #
notify_pipeline_status_op.component_spec.implementation.container.image_pull_policy = "Always" #
...
def training_pipeline(
    ...
    kfp_components_image_uri_param: str = KFP_COMPONENTS_IMAGE_URI, #
    vertex_lstm_trainer_image_uri_param: str = VERTEX_LSTM_TRAINER_IMAGE_URI, #
    ...
    pipeline_ui_link_val = f"https://console.cloud.google.com/vertex-ai/locations/{REGION}/pipelines/runs/{pipeline_job_id_val}?project={PROJECT_ID}" # Use REGION

    exit_notify_task = notify_pipeline_status_op(
        project_id=PROJECT_ID, #
        pair=pair, #
        timeframe=timeframe, #
        pipeline_run_status='{{$.pipeline_task_status}}', # KFP will fill this with Succeeded/Failed/Cancelled
        pipeline_job_id=pipeline_job_id_val, #
        pipeline_job_link=pipeline_ui_link_val, #
        model_promoted=False #
    )
    exit_notify_task.set_display_name("Notify_Pipeline_Final_Status_Exit_Handler") #
    # No .set_container_image() here; it uses the base_image from @dsl.component
    ...
    with dsl.ExitHandler(exit_notify_task): #
        ...
        update_data_task.set_container_image(kfp_components_image_uri_param) #
        ...
        prepare_opt_data_task.set_container_image(kfp_components_image_uri_param) #
        ...
        optimize_lstm_task.set_container_image(kfp_components_image_uri_param) #
        ...
        train_lstm_task = train_lstm_vertex_ai_op(
            ...
            vertex_training_image_uri=vertex_lstm_trainer_image_uri_param, #
        ...
        prepare_rl_data_task.set_container_image(kfp_components_image_uri_param) #
        ...
        train_rl_task.set_container_image(kfp_components_image_uri_param) #
        ...
        backtest_task.set_container_image(kfp_components_image_uri_param) #
        ...
        decide_task.set_container_image(kfp_components_image_uri_param) #
        decide_task.set_display_name("Decide_Model_Promotion") #
        decide_task.set_cpu_limit("2").set_memory_limit("4G") #

        with dsl.If(decide_task.output == True, name="If_Model_Promoted_Notify_Success"): #
            ...
            notify_promoted_task.after(decide_task) #

        with dsl.If(decide_task.output == False, name="If_Model_Not_Promoted_Notify_Success"): #
            ...
            notify_not_promoted_task.after(decide_task) #
...
if __name__ == "__main__":
    ...
    if SUBMIT_TO_VERTEX_AI: #
        ...
        aiplatform_sdk.init(
            project=PROJECT_ID, #
            location=REGION, #
            service_account=DEFAULT_VERTEX_LSTM_SERVICE_ACCOUNT #
        )
        print(f" Vertex AI SDK inicializado para Project: {PROJECT_ID}, Region: {REGION}, Service Account: {DEFAULT_VERTEX_LSTM_SERVICE_ACCOUNT}") #
        ...
        pipeline_parameter_values = {
            ...
            "kfp_components_image_uri_param": KFP_COMPONENTS_IMAGE_URI, #
            "vertex_lstm_trainer_image_uri_param": VERTEX_LSTM_TRAINER_IMAGE_URI, #
        ...
        try:
            ...
            if job and hasattr(job, 'resource_name') and job.resource_name: #
                 ...
                 try:
                     current_job_state = job.state if hasattr(job, 'state') and job.state else "No disponible (falló antes de tener estado)" #
                     current_job_error = job.error if hasattr(job, 'error') and job.error else None #
                     print(f"  Estado actual del Job: {current_job_state}") #
                     if current_job_error: #
                         error_message = getattr(current_job_error, 'message', str(current_job_error)) #
                         print(f"  Error del Job: {error_message}") #
                 except Exception as refresh_err: #
                     print(f"  No se pudo obtener el estado detallado del job tras el fallo: {refresh_err}") #
            else: #
            ...
            if job_resource_name: #
                ...
                pipeline_console_link = f"https://console.cloud.google.com/vertex-ai/locations/{REGION}/pipelines/runs/{job_id_for_link}?project={PROJECT_ID}" #
