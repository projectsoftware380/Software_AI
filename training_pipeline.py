    ...
    if process.returncode != 0:
        ...
        try:
            ...
            if blob.exists():
                ...
                # Asumiendo que metrics_data es un diccionario y queremos loguear las métricas del modelo "filtrado"
                # y no del modelo "base". Si la estructura es diferente, ajustar aquí.
                metrics_to_log = metrics_data.get("filtered", metrics_data) # Prioriza 'filtered'
...
def decide_promotion_op(
    ...
    command = [
        ...
        "--pair", pair,
        "--timeframe", timeframe,
        # Podrías necesitar pasar --project-id o --gcs-bucket-name a model_promotion_decision.py si los usa
    ]
    logger.info(f"Executing command: {' '.join(command)}")
    process = subprocess.run(command, capture_output=True, text=True, check=False)
...
def notify_pipeline_status_op(
    ...
    try:
        ...
        # KFP v2 usa estados como "SUCCEEDED", "FAILED", "CANCELLED"
        if status_summary not in ["SUCCEEDED", "FAILED", "CANCELLED", "SUCCESS", "FAILURE"]: # Añadido SUCCESS/FAILURE por si acaso
            ...
            if "fail" in pipeline_run_status.lower() or "error" in pipeline_run_status.lower():
                status_summary = "FAILED" # Usar FAILED para consistencia con KFPv2
            elif "success" in pipeline_run_status.lower() or "succeeded" in pipeline_run_status.lower():
                status_summary = "SUCCEEDED" # Usar SUCCEEDED para consistencia con KFPv2
            else:
                status_summary = "UNKNOWN_STATUS" # Si no se puede determinar, marcar como desconocido
        ...
        if status_summary == "UNKNOWN_STATUS": # Si es desconocido, tratar como fallo para notificación
...
def training_pipeline(
    ...
    vertex_lstm_training_image: str = VERTEX_LSTM_TRAINER_IMAGE_URI, # Usará la imagen runner definida globalmente
    ...
    kfp_opt_accelerator_type: str = KFP_COMPONENT_ACCELERATOR_TYPE, # No se usa en optimize_lstm_op actualmente
    kfp_opt_accelerator_count: int = KFP_COMPONENT_ACCELERATOR_COUNT, # No se usa en optimize_lstm_op actualmente
    ...
    # El pipeline_job_name_val no se está usando en las notificaciones, pero es bueno tenerlo.
    ...
    with dsl.ExitHandler(
        notify_pipeline_status_op(
            ...
            pair=pair,
            timeframe=timeframe,
            pipeline_run_status="UNKNOWN_STATUS",  # CORREGIDO para usar el placeholder KFP v2
            pipeline_job_id=pipeline_job_id_val,
            pipeline_job_link=pipeline_ui_link_val,
            model_promoted=False # Por defecto False, se actualiza si el pipeline tiene éxito y promociona
        )
    ):
        ...
        # Nota: kfp_opt_accelerator_type y kfp_opt_accelerator_count no se pasan a optimize_lstm_op.
        # Si se necesitaran GPUs para este componente KFP (no para el Custom Job que es diferente),
        # se configurarían con .set_gpu_limit() o similar, pero el componente mismo debe soportarlo.
        ...
        train_lstm_task.set_cpu_limit("1").set_memory_limit("2G") # Este es el límite para el lanzador KFP, no para el Custom Job
        ...
        with dsl.If(decide_task.output == True, name="If_Model_Promoted"):
            notify_promoted_task = notify_pipeline_status_op(
                ...
                pair=pair,
                timeframe=timeframe,
                pipeline_run_status="SUCCEEDED", # Estado explícito para esta rama
                model_promoted=True,
                pipeline_job_id=pipeline_job_id_val,
                pipeline_job_link=pipeline_ui_link_val,
            )
        ...
        with dsl.If(decide_task.output == False, name="If_Model_Not_Promoted"):
            notify_not_promoted_task = notify_pipeline_status_op(
                ...
                pair=pair,
                timeframe=timeframe,
                pipeline_run_status="SUCCEEDED", # Estado explícito para esta rama (pipeline tuvo éxito, pero no se promocionó)
...
if __name__ == "__main__":
    ...
    if SUBMIT_TO_VERTEX_AI:
        ...
        # Corregir el typo en el nombre del parámetro si existe (aunque ya no debería estar)
