...
Responsabilidades:
1.  Cargar los hiperparámetros del archivo `params.json` (generado durante la
    optimización y copiado por el paso de entrenamiento del LSTM).
2.  Cargar el dataset para RL (archivo .npz) que contiene las observaciones
    y las señales base del LSTM.
3.  Definir un entorno de `gymnasium` (`SignalFilterEnv`) donde el agente
    aprende a tomar decisiones (aceptar/rechazar señal).
4.  Configurar y entrenar un agente PPO (Proximal Policy Optimization) de
    `stable-baselines3`.
5.  Guardar el modelo RL entrenado (un archivo .zip) en una nueva carpeta
    versionada por timestamp en GCS.

Reemplaza la funcionalidad de `train_rl.py`.
"""
...
class SignalFilterEnv(Env):
    """
    Entorno para que un agente PPO aprenda a filtrar señales de un LSTM.
    
    La acción es discreta: 0 (rechazar) o 1 (aceptar).
    La recompensa se obtiene si se acepta una señal que resulta ser rentable.
    Se aplica una pequeña penalización por aceptar señales inútiles.
    """
    ...
    def step(self, action: int):
        ...
        # Penalizar acciones que no llevan a nada para evitar que el agente
        # aprenda a aceptar todo indiscriminadamente.
...
def run_rl_training(
    ...
    """
    Orquesta el proceso completo de entrenamiento del agente PPO.
    Retorna la ruta GCS del modelo .zip final.
    """
    try:
        # 1. Cargar artefactos
        ...
        # 2. Configurar y entrenar el agente PPO
        ...
        # Configurar ruta de logs para TensorBoard
        ...
        # 3. Guardar el modelo entrenado en GCS
...
if __name__ == "__main__":
    ...
    parser.add_argument("--params-path", required=True, help="Ruta GCS al JSON de hiperparámetros.")
    parser.add_argument("--rl-data-path", required=True, help="Ruta GCS al archivo .npz con datos para RL.")
